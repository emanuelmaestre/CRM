...
const data=await (p as unknown as {request(rota:string,opcoes:{query:Record<string,string>}):Promise<{orders?:Array<Record<string,unknown>&{id:string;status:string;payment?:object;cancel_reason?:string}>}>}).request('/order/202309/orders',{query:{ids:'585879432666907856,585777470352033313'}});
...
const {shopeeAppEnvSuffix}=await import('../src/shared/config/shopee-env');
