 ...
 for(const canal of ['shopee','tiktokshop'] as const){
  ...
  try{
   ...
   for(const n of [0,6,29]){
    ...
    console.log(JSON.stringify({marca:slug,canal,periodo:n===0?'hoje':n+1,apiPedidos:list.length,apiTotalTodos:list.reduce((s,p)=>s+Math.round(Number(p.total)*100),0)/100,apiFaturaveis:list.filter(p=>statusPedidoFaturavel(mapearStatusPedido(p.status))).length,apiValorFaturaveis:list.filter(p=>statusPedidoFaturavel(mapearStatusPedido(p.status))).reduce((s,p)=>s+Math.round(Number(p.total)*100),0)/100,cancelados:cancels.length,canceladosValor:cancels.reduce((s,p)=>s+Math.round(Number(p.total)*100),0)/100,canceladosMarcador:cancels.map(p=>(byId.get(p.providerOrderId)?.dados_origem as {pagamentoAprovado?:unknown}|null)?.pagamentoAprovado??'ausente'),missing:missing.map(p=>({id:p.providerOrderId,total:p.total,status:p.status})),differences,extras:extras.map(p=>({id:p.provider_order_id,total:p.total,status:p.status}))}));
   }
