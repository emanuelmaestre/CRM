...
const r=await shopeeFetch((p as any).urlPedidos('/order/get_order_detail',{order_sn_list:rows.map(r=>r.provider_order_id).join(','),response_optional_fields:'pay_time,cancel_reason,total_amount'}));
const j=await r.json();console.log(JSON.stringify({http:r.status,error:j.error,orders:j.response?.order_list?.map((o:any)=>({id:o.order_sn,status:o.order_status,pay_time:o.pay_time,cancel_reason:o.cancel_reason,keys:Object.keys(o)}))}));process.exit(0);
