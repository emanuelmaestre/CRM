...
for(const slug of process.argv.slice(2)){
 ...
 for(const n of [0,7,30]){
  ...
  const wrongDates=official.flatMap(p=>{const l=byId.get(String(p.id));if(!l)return[];const d=l.dados_origem as any;const t=typeof d?.aprovadoEmMs==='number'?d.aprovadoEmMs:Date.parse(String(l.criado_em));return t<inicio.getTime()||t>cutoff.getTime()?[{id:String(p.id),total:p.total_amount,criacao:l.criado_em,aprovacao:aprovacaoMercadoLivre(p)}]:[];});
  ...
  const extras=locals.filter(l=>{const d=l.dados_origem as any;const t=typeof d?.aprovadoEmMs==='number'?d.aprovadoEmMs:Date.parse(String(l.criado_em));return t>=inicio.getTime()&&t<=cutoff.getTime()&&d?.cancelamento?.code!=='pack_splitted'&&(l.status==='pago'||d?.pagamentoAprovado||Number(d?.valorPago)>0)&&!officialIds.has(String(l.provider_order_id));}).map(l=>({id:l.provider_order_id,total:l.total,status:l.status}));
  const row={periodo:n||'hoje',inicio:diaAtras(n),fim:cutoff.toISOString(),crm:{bruto:crm.totalBrutoComparavel,pedidos:crm.totalBrutoPedidos},api:{bruto:gross,pedidos:official.length,unidades:official.reduce((s,p)=>s+p.order_items.reduce((v:number,i:any)=>v+i.quantity,0),0)},diferenca:Math.round((gross-crm.totalBrutoComparavel)*100)/100,missing,wrongDates,valueDiff,extras,semDataAprovacao:official.filter(p=>{const l=byId.get(String(p.id));return l&&typeof (l.dados_origem as any)?.aprovadoEmMs!=='number';}).length};
  rows.push(row);
