...
describe("consulta de pedidos", () => {
  it("normaliza filtros externos para o caso de uso", () => {
    ...
    expect(consulta).toMatchObject({ canais: ["mercadolivre"], statuses: ["pago", "entregue"], busca: "pedido 123", offset: 50 });
  ...
  it("remove filtros vazios e usa offset zero", () => {
    expect(normalizarConsultaPedidos({ brandIds: [], canais: [], busca: "" })).toEqual({ offset: 0 });
  });

  it("rejeita canal, status e identificadores inválidos", () => {
    expect(() => normalizarConsultaPedidos({ canais: ["whatsapp"] })).toThrow();
