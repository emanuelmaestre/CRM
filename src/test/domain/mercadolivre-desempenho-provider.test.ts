...
const pedido = (id: number, date_created: string, status = "paid", quantity = 1) => ({
...
describe("fontes oficiais dos oito cards", () => {
  it("mantém a conferência em Brasília e alinha pedidos/visitas ao calendário UTC-4", async () => {
    const { provider, chamadas } = preparar([
      ...
      pedido(3, "2026-08-30T18:00:00Z", "cancelled", 3),
      pedido(4, "2026-08-31T03:30:00Z", "returned", 1), // Só desempenho.
    ]);
    const resumo = await provider.resumirFaturamentoOficial(inicio, fim, true, agora);
    expect(resumo).toMatchObject({ faturamento: 20.2, totalPedidos: 3, canceladosQtd: 1, totalBruto: 30.3,
