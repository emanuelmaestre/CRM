...
export type FaturamentoOficial =
  | {
      status: "ok";
      faturamento: number;
  ...
  | { status: "indisponivel" | "nao_aplicavel"; mensagem: string };
...
export function ConferenciaCanal({
  ...
  const mercadoLivre = canal === "mercadolivre";
  ...
  const oficialOk = mercadoLivre && faturamentoOficial?.status === "ok" ? faturamentoOficial : null;
    ...
    <motion.section layout className="mb-4 overflow-hidden rounded-[1.25rem] border border-border bg-card">
      <button type="button" onClick={() => setAberto((atual) => !atual)} aria-expanded={aberto} aria-controls={id} className="press-feedback flex w-full flex-wrap items-center gap-x-3 gap-y-1.5 px-4 py-3 text-left transition-colors hover:bg-muted/40">
        <span className="grid size-8 shrink-0 place-items-center rounded-full" style={{ background: "color-mix(in srgb, var(--info) 12%, transparent)", color: "var(--info)" }}>
          {mercadoLivre ? <Radio size={16} /> : <Scale size={16} />}
        </span>
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-1.5 text-[13px] font-bold text-foreground">
            {mercadoLivre ? "Mercado Livre ao vivo" : "Entenda os totais do CRM"}
            <ChannelLogo canal={canal} size="xs" variant="logo" />
            <span className="sr-only"> — {nome}</span>
          </span>
          <span className="block text-[11.5px] text-muted-foreground">
            {mercadoLivre ? "Consulta autenticada diretamente na API oficial de pedidos." : "Estes valores não são uma conferência com o painel oficial."}
          </span>
        ...
        <span className="flex w-full shrink-0 items-center justify-between gap-3 pl-11 sm:ml-auto sm:w-auto sm:justify-start sm:gap-4 sm:pl-0">
          <span className="text-left sm:text-right">
            <span className="block text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
              {mercadoLivre ? "Faturamento ao vivo · Mercado Livre" : "Faturamento atual no CRM"}
            </span>
            <span data-testid={mercadoLivre ? "faturamento-oficial-mercadolivre" : "faturamento-atual-crm"} aria-live="polite" className="block text-sm font-bold tabular-nums text-foreground">
              {valorCabecalho}
            </span>
          </span>
          <span className="text-[11.5px] font-semibold text-muted-foreground">{aberto ? "Ocultar conferência" : "Ver conferência"}</span>
            ...
            <div className="space-y-3 border-t border-border px-4 py-3 text-[12px] leading-relaxed text-muted-foreground">
              {!dadosAtuais ? (
                ...
                <>
                  {oficialOk ? (
                    <>
                      <dl className="space-y-2 text-[13px]">
                        <div className="flex flex-wrap justify-between gap-2"><dt>Mercado Livre ao vivo ({oficialOk.pedidosValidos} pedidos válidos)</dt><dd className="font-semibold tabular-nums text-foreground">{moeda.format(oficialOk.faturamento)}</dd></div>
                        ...
                        {!temFiltrosAdicionais && <div className="flex flex-wrap justify-between gap-2 border-t border-border pt-2 font-bold text-foreground"><dt>Diferença (CRM − Mercado Livre)</dt><dd data-testid="diferenca-faturamento" className="tabular-nums">{moeda.format(diferenca ?? 0)}</dd></div>}
                      ...
                      {!temFiltrosAdicionais && diferenca === 0 && <p className="font-semibold text-foreground">O faturamento comparável do CRM está igual ao Mercado Livre neste momento.</p>}
                  ...
                  <p>Esta leitura vem do endpoint oficial de pedidos do Mercado Livre no período selecionado. Ela compara a mesma regra de faturamento do CRM; não se apresenta como reprodução do indicador separado “Vendas brutas” do painel, cuja fórmula pode ser diferente.</p>
                </>
              ) : (
                <>
                  <dl className="space-y-2 text-[13px]">
