  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold">{copy.orderPrefix}{detalhe.providerOrderId ?? detalhe.id.slice(0, 8)}</h1><p className="mt-1 text-sm text-muted-foreground capitalize">{detalhe.canal}</p></div>
      ...
      <dl className="mt-6 grid gap-3 rounded-2xl border border-border bg-card p-5 sm:grid-cols-2 lg:grid-cols-4">
        {[[copy.customer, detalhe.clienteNome], [copy.brand, detalhe.brandNome], [copy.channel, detalhe.canal], [copy.externalAccount, detalhe.contaNome ?? copy.legacyAccount]].map(([label, value]) => <div key={label}><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 font-semibold capitalize">{value}</dd></div>)}
      </dl>
      <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-card">
        <h2 className="border-b border-border px-5 py-4 font-bold">{copy.items}</h2>
        {itens.length === 0 ? <p className="p-5 text-sm text-muted-foreground">{copy.emptyItems}</p> : <div className="divide-y divide-border">{itens.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 p-5"><div><p className="font-semibold">{item.nome}</p><p className="text-xs text-muted-foreground">{item.sku} · {copy.quantity}: {item.quantidade}</p></div><span className="tabular-nums">{moeda(item.precoUnitario)}</span></div>)}</div>}
