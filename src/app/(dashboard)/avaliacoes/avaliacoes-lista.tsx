...
export function AvaliacoesLista({ marcasAtivas, canaisAtivos, onContagens, itensIniciais }: {
  ...
  const falta = oQueFaltaNoEscopo(marcasAtivas, canaisAtivos);
  const semFiltro = escopoIncompleto(marcasAtivas, canaisAtivos);
    ...
    return (
      <div className="overflow-hidden rounded-[1.25rem] border border-border bg-card shadow-[0_2px_16px_rgba(14,15,19,.06)]">
        <EmptyState
          ...
          title={conviteDeEscopo(falta ?? "ambos").titulo}
          description={conviteDeEscopo(falta ?? "ambos").descricao}
        />
      </div>
    );
  }
